...
def norm_plot_n(L, Label, x, p, s, model="norm"): #L[n, M] = matrice of the distances for all n=edges  
    n = len(L)
    m = len(Label)
    R = auxiliaire_norm_plot(L)
    l = np.arange(0, n, 1)
    ...
    if model=="norm": plt.ylabel('|| A - PBtP||')
    if model=="trace": plt.ylabel('tr(P^T A^T P B)')
    if model=="overlap": plt.ylabel('ov(P, Id)')
